...
namespace service {
    class CachedProject : public ProjectBase, CacheableServiceBase {
    public:
        explicit CachedProject(const std::shared_ptr<ProjectBase> &wrapped);
        ...
        std::optional<std::string> readLangKey(const std::string &namespace_, const std::string &key) const override;
        TaskResult<FileTree> getDirectoryTree() const override;
        TaskResult<FileTree> getContentDirectoryTree() const override;
    ...
    private:
        ...
        std::shared_ptr<ProjectBase> wrapped_;
    };
}
