...
namespace service {
    ...
    Task<std::vector<ProjectIssue>> Database::getDeploymentIssues(const std::string deploymentId) const {
        const auto res = co_await handleDatabaseOperation([deploymentId](const DbClientPtr &client) -> Task<std::vector<ProjectIssue>> {
            CoroMapper<ProjectIssue> mapper(client);
    ...
    Task<std::unordered_map<std::string, int64_t>> Database::getActiveProjectIssueStats(std::string projectId) const {
        ...
        const auto res =
            co_await handleDatabaseOperation([projectId](const DbClientPtr &client) -> Task<std::unordered_map<std::string, int64_t>> {
