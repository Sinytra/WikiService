...
const std::string Recipe::Cols::_type_id = "\"type_id\"";
...
{"type_id","int64_t","bigint",8,0,0,1}
};
...
Recipe::Recipe(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[3].empty() && pJson.isMember(pMasqueradingVector[3]))
    {
        ...
        {
            typeId_=std::make_shared<int64_t>((int64_t)pJson[pMasqueradingVector[3]].asInt64());
        }
    }
}
...
void Recipe::setVersionIdToNull() noexcept
{
    versionId_.reset();
    dirtyFlag_[1] = true;
}
...
const int64_t &Recipe::getValueOfTypeId() const noexcept
{
    static const int64_t defaultValue = int64_t();
    if(typeId_)
        return *typeId_;
...
const std::shared_ptr<int64_t> &Recipe::getTypeId() const noexcept
{
    return typeId_;
}
void Recipe::setTypeId(const int64_t &pTypeId) noexcept
{
    ...
    const std::vector<std::string> &pMasqueradingVector) const
{
    ...
    if(pMasqueradingVector.size() == 4)
    {
        ...
        if(!pMasqueradingVector[3].empty())
        {
            if(getTypeId())
            {
                ret[pMasqueradingVector[3]]=(Json::Int64)getValueOfTypeId();
            }
            else
            {
